...
SignalSet::SignalSet(EventLoop* loop,
    ...
    : loop_(loop),
      ...
      signalfd_(CreateSignalfd(-1, signals_)),
      channel_(new Channel(loop, fd_))
{
    signalfdChannel_->setLowPriority();
    signalfdChannel_->setReadCallback(
        boost::bind(&SignalSet::OnSignal, this));
    signalfdChannel_->enableReading();
}
...
void SignalSet::Add(int signal_number)
{
    ...
    if (signalfd_ != -1 && signals_.size() == 1)
    {
